...
class Forecaster(nn.Module):
    ...
    def get_input_sequence(
        ...
        if mode == 'all':
            ...
            # decode_len = 1 if self.autoregressive else self.prediction_length
            # future_token = torch.zeros(
            #     past_target_cdf.shape[0], decode_len, past_target_cdf.shape[-1])
            # sequence = torch.cat(
            #     [past_target_cdf, future_token.to(past_target_cdf.device)], dim=1)
            # seq_length = decode_len
            sequence = past_target_cdf
            seq_length = 1
        else:
